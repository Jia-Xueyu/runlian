...
while true,
    ...
    [sw1,sw2]=rr_record( tr_width,rr_f1,rr_f2,dt,record_time,fskSigWithNoise );%接收方记录
    ...
    %判断
    if (length(find(xor(V, validation) == 1))/length(validation))<0.35,
        fprintf('ACK count %d\n', count);
        count=count+1;
        Final=[Final F];
        while true,
        ...
        index=index-send_every;
        fprintf('change count %d\n', change_count);
        change_count=change_count+1;
        [s1,s2 ]= rr_change( rs_f1,rs_f2,sr_f1,sr_f2,dt,tr_width );
        signal=decode( s1,s2,5,bitSampleNum );
        if signal==[1,0,0,0,1],
            ss_f1 = 17000;                                 
            ss_f2 = 18000; 
            sr_f1=18000;
