...
class myTransmitter(threading.Thread):
	def __init__(self,start,s_f1,s_f2,r_f1,r_f2,next,index,signalnum,decode_index,code,preamble,signal,information,ack,ack_code,trans_again,record_notation):
		self.start=start
		self.s_f1=s_f1
		self.s_f2=s_f2
		self.r_f1=r_f1
	...
	def transmit(self):
		...
		if self.trans_again==0:
			...
			if (self.newtime-self.oldtime).seconds>60:
				self.oldtime=datetime.now();
				print('reaudio')
